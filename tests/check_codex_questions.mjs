...
function fixture(){
 ...
 function node(tag='div'){let text='';return{tagName:String(tag).toUpperCase(),value:'',style:{},disabled:false,children:[],scrollHeight:10,clientHeight:10,scrollTop:0,classList:{contains:()=>false},appendChild(x){this.children.push(x)},closest(){return{classList:{contains:()=>true}}},set textContent(v){text=String(v);if(v==='')this.children=[]},get textContent(){return text+this.children.map(x=>x.textContent||'').join('')}}}
 ...
 const context=vm.createContext({window:{},URL,document:{hidden:false,getElementById:$,createElement:node,createTextNode:text=>({textContent:String(text)}),addEventListener(){}},localStorage:{getItem:k=>stored.get(k)||null,setItem:(k,v)=>stored.set(k,v)},setInterval:f=>{poll=f;return 1},clearInterval(){}});
 vm.runInContext(markdown,context);context.CodexMarkdown=context.window.CodexMarkdown;vm.runInContext(source,context);const q=context.window.CodexQuestions;q.init(deps);
...
}
{
 ...
 const message=f.$('codex-conversation').children.at(-1),body=message.children[1];assert(body.textContent.startsWith('<img'),'Bot text is displayed literally');const link=body.children.flatMap(x=>x.children||[]).find(x=>x.href);assert.equal(link.href,'https://chatgpt.com/codex/tasks/test');
}
{
